...
- ⛔ **L3 真付受阻于 Coinsnap store 未绑钱包**(非我方代码):凭据有效(`GET /api/v1/stores/{id}` → HTTP 200,store "Hygpo - graphic",provider=coinsnap;容器出网到 `app.coinsnap.io` 正常),但 `POST /api/v1/stores/{id}/invoices` → **HTTP 500 "An error occurred while trying to create invoice."**。$0 与 $1 均 500。原因几乎可以确定是 **app.coinsnap.io 上该 store 还没连接收款 Lightning 钱包/地址** → 无收款目标 → 开发票失败。**待办(Coinsnap 账号侧,非服务器/非代码)**:登录 app.coinsnap.io → store「Hygpo - graphic」→ 连接钱包/闪电地址(或启用 Coinsnap 托管 lightning),之后 invoice 应从 500 变 200,L3 真付即可走通。
